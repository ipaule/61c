/*********************
**  Mandelbrot fractal
** clang -Xpreprocessor -fopenmp -lomp -o Mandelbrot Mandelbrot.c
** by Dan Garcia <ddgarcia@cs.berkeley.edu>
** Modified for this class by Justin Yokota and Chenyu Shi
**********************/

#include <stdio.h>
#include <stdlib.h>
#include "ComplexNumber.h"
#include "Mandelbrot.h"
#include <sys/types.h>

/*
This function returns the number of iterations before the initial point >= the threshold.
If the threshold is not exceeded after maxiters, the function returns 0.
*/
u_int64_t MandelbrotIterations(u_int64_t maxiters, ComplexNumber * point, double threshold)
{
    //YOUR CODE HERE
    u_int64_t count = 0;
    ComplexNumber* z = newComplexNumber(0,0);
    while(ComplexAbs(z) < threshold){
      ComplexNumber* prod  = ComplexProduct(z,z);
      ComplexNumber* added = ComplexSum(prod,point);
      freeComplexNumber(z);
      z  = newComplexNumber(Re(added),Im(added));
	    freeComplexNumber(prod);
      freeComplexNumber(added);
      count++;
      if(count == maxiters){
        freeComplexNumber(z);
        return 0;
      }
    }
     freeComplexNumber(z);
  return count;
}

/*
This function calculates the Mandelbrot plot and stores the result in output.
The number of pixels in the image is resolution * 2 + 1 in one row/column. It's a square image.
Scale is the the distance between center and the top pixel in one dimension.
*/
void Mandelbrot(double threshold, u_int64_t max_iterations, ComplexNumber* center, double scale, u_int64_t resolution, u_int64_t * output){
    //YOUR CODE HERE
    int i, j;
    double gap = scale/resolution;
    ComplexNumber* forFirst = newComplexNumber((-1)*scale, scale);
    ComplexNumber* added = ComplexSum(center, forFirst);
    ComplexNumber* first = newComplexNumber(Re(added), Im(added));
    ComplexNumber* next;
    freeComplexNumber(forFirst);
    freeComplexNumber(added);
    for(i = 0; i < 2*resolution+1; i++){
      for(j = 0; j < 2*resolution+1; j++){
        next = newComplexNumber(j*gap,(-1)*i*gap);
        added = ComplexSum(first, next);
        *(output + i*(2*resolution+1) + j) = MandelbrotIterations(max_iterations, added, threshold);
        freeComplexNumber(added);
        freeComplexNumber(next);
      }
    }
    freeComplexNumber(first);
}
